...
-   [Prev](systemd-units.md "BLFS Systemd Units"){accesskey="p"}

    BLFS Systemd Units

-   [Next](libraries.md "Libraries: Static or shared?"){accesskey="n"}
...
# []{#la-files}About Libtool Archive (.la) files {#about-libtool-archive-.la-files .sect1}

:::: {.sect1 lang="en"}
::: {.package lang="en"}
## Files with a .la extension {#files-with-a-.la-extension .sect2}
...
The solution is to remove the .la files. However there is a catch. Some packages, such as [ImageMagick-7.1.2-13](../general/imagemagick.md "ImageMagick-7.1.2-13"){.xref}, use a libtool function, lt_dlopen, to load libraries as needed during execution and resolve their dependencies at run time. In this case, the .la files should remain.
